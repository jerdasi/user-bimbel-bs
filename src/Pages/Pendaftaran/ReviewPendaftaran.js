...
function ReviewPendaftaran() {
    ...
    const handleDaftar = (e) => {
        ...
        if (pendaftaran.id_grup === 0) {
            ...
            axios
                .post(
                    `${process.env.REACT_APP_API}/pendaftaran/pendaftaran-midtrans`,
                    pendaftaran
                )
                .then((res) => {
                    navigate(`/rincian-pembayaran`, {
                        state: {
                            id_paket: id_paket,
                            id_grup: idGrup,
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then((res) => {
                ...
                setPendaftaran({
                    ...pendaftaran,
                    total_pembayaran:
                        res.data.data.harga + pendaftaran.total_pembayaran,
                });
        ...
        axios
            .get(`${process.env.REACT_APP_API}/jadwal-grup`)
            .then((res) => setJadwalGrup(res.data.data));
        axios
            .get(`${process.env.REACT_APP_API}/hari-jam`)
            .then((res) => setHariJam(res.data.data));
        axios
        ...
        <div className="p-4 md:p-16">
            ...
            <div className="w-full md:flex justify-start">
                <div className="w-full md:w-1/3 p-2 border-2 border-red-600 rounded-md max-h-72">
                    <img
                        src={`${process.env.REACT_APP_API}/${review.foto}`}
                        className="p-2 md:w-[210px] md:h-[220px] mx-auto"
                    />
                    <button className="flex  p-2 w-full bg-merah-bs text-white md:text-lg rounded-md my-2">
                        <BsSearch size={20} className="my-1" />{" "}
            ...
            <div>
                <h3 className="font-bold py-2 text-xl md:text-2xl">
                    Paket yang <span className="text-merah-bs">dipilih</span>
                </h3>
                ...
                <div className="py-6">
                    <h2 className="font-bold text-xl md:text-2xl">
                        Pilihan{" "}
                        <span className="text-merah-bs">Grup Bimbingan </span>
                    </h2>
                    <div className="flex gap-2 h-64 flex-col flex-wrap overflow-x-auto disabled">
                        {grup
                            .filter((item) => item.id_paket === id_paket)
                            .map((item) => {
                                return (
                                    <div
                                        key={item.id}
                                        className={[
                                            "rounded-md border-2 border-red-600 p-4 w-full md:w-1/3 cursor-pointer h-full",
                                            item.kuota == 0
                                                ? "hidden"
                                                : "block",
                                        ]}
                                    >
                                        ...
                                        <button
                                            ...
                                            className={[
                                                "w-full flex p-2 px-6 justify-between items-center border-red-600 border-2 rounded-lg",
                                                item.id === pendaftaran.id_grup
                ...
                <div>
                    ...
                    />
                    Saya setuju untuk mengikuti segala aturan dan ketentuan yang
                    berlaku di Bimbingan Belajar Beta Smart
                    <div className="flex flex-wrap md:flex-nowrap gap-2">
                        <button
                            className="w-full md:w-1/2 mt-1 p-2 px-6 justify-between items-center border-2 border-gray-600 font-bold rounded-md"
                            onClick={(e) => {
                        ...
                        <button
                            ...
                            className="w-full md:w-1/2 mt-1 p-2 px-6 justify-between items-center text-white font-bold bg-merah-bs rounded-md"
                        >
                            Lanjutkan dan Bayar{" "}
                        </button>
                    </div>
